...
const activeCircleStyles =
  "flex items-center justify-center w-3 h-3 rounded-full transition-colors duration-300";

export function ThemeSwitcher() {
  const [theme, setTheme] = useState("light");

  const ChangeThemeHundler = (theme) => {
    if (!theme) return;
    ...
    localStorage.setItem("around-the-world-theme", theme);
    setTheme(theme);
  };

  useEffect(() => {
    const localTheme = localStorage.getItem("around-the-world-theme");
    ChangeThemeHundler(localTheme);
  }, []);

  return (
    <div className="flex h-6 w-11 items-center justify-around rounded-full bg-gray-800 transition-colors duration-300 dark:bg-gray-900">
      <button
        className={`${activeCircleStyles} ${theme === "light" ? "bg-white" : ""}`}
        onClick={() => ChangeThemeHundler("light")}
      >
        <svg
          width="10"
          height="10"
        ...
        >
          <path
            className="fill-gray-800 transition-colors duration-300 dark:fill-gray-100"
      ...
      <button
        ...
        onClick={() => ChangeThemeHundler("dark")}
      >
        <svg
          width="7.5"
          height="7.5"
        ...
        >
          <path
            ...
            fill="#F9FAFB"
            className="fill-gray-100 transition-colors duration-300 dark:fill-gray-900"
          />
        </svg>
      </button>
    </div>
