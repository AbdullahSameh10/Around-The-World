...
export function DetailsPage() {
  ...
  const { countriesList, isLoading, isError } = useFetchData(country);
      ...
      {!isLoading && !isError && (
        <div className="">
          ...
          <div className="grid gap-11 lg:grid-cols-2 lg:gap-36">
            <img
              className="w-full"
              src={countriesList?.flags?.svg}
              alt={countriesList?.flags?.alt}
            />
            <div>
              <h1 className="mb-4 text-3xl font-extrabold lg:mb-7">
                {countriesList?.name?.common}
              </h1>
              <div className="flex flex-col gap-8 md:gap-40 lg:flex-row">
                <div className="flex flex-col gap-5">
                  <p>
                    ...
                    <span className="font-light">
                      {parseInt(countriesList?.population).toLocaleString()}
                    </span>
                  </p>

                  <p>
                    <span className="font-semibold">Region: </span>
                    <span>{countriesList?.region}</span>
                  </p>

                  <p>
                    <span className="font-semibold">Sub Region: </span>
                    <span>{countriesList?.subregion}</span>
                  </p>

                  <p>
                    <span className="font-semibold">Capital: </span>
                    <span>{countriesList?.capital}</span>
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-5">
                  <p>
                    ...
                    <span>{countriesList?.tld?.join(", ")}</span>
                  </p>

                  <p>
                    <span className="font-semibold">Currencies: </span>
                    <span className="font-thin">
                      {countriesList?.currencies &&
                        Object.values(countriesList?.currencies)
                  ...
                  <p>
                    ...
                    <span className="font-thin">
                      {countriesList?.languages &&
                        Object.values(countriesList?.languages)
